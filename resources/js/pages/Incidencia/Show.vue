  ...
  <AppLayout title="Detalle de Incidencia">
    ...
    <div class="px-4 py-6 sm:px-6 lg:px-8">
      <div class="flex gap-2 mb-6">
        <Link href="/incidencias" class="inline-flex items-center px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <Card class="lg:col-span-2">
          <CardHeader>
            <CardTitle>Informacion de la Incidencia</CardTitle>
          </CardHeader>
          <CardContent class="space-y-4">
            <div>
              <Label class="text-gray-600 font-semibold">Descripcion</Label>
              <p class="text-gray-900 mt-1">{{ incidencia.descripcion_problema }}</p>
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <Label class="text-gray-600 font-semibold">Usuario</Label>
                <p class="text-gray-900 mt-1">{{ incidencia.empleado.usuario.nombres + ' ' + incidencia.empleado.usuario.apellidos || 'N/A' }}</p>
              </div>
              <div>
                <Label class="text-gray-600 font-semibold">Categoria</Label>
                <p class="text-gray-900 mt-1">{{ incidencia.categoria?.descripcion || 'N/A' }}</p>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <Label class="text-gray-600 font-semibold">Fecha de Creacion</Label>
                <p class="text-gray-900 mt-1">{{ formatDate(incidencia.fecha_incidencia) }}</p>
              </div>
              <div>
                <Label class="text-gray-600 font-semibold">Estado</Label>
                <span :class="getEstadoBadgeClass(incidencia.estado)" class="inline-block px-3 py-1 rounded-full text-xs font-semibold mt-1">
        ...
        <Card>
          ...
          <CardContent class="space-y-4">
            <div>
              <Label class="text-gray-600 text-sm font-semibold">Prioridad</Label>
              <span :class="getPrioridadBadgeClass(incidencia.prioridad)" class="inline-block px-3 py-1 rounded-full text-xs font-semibold mt-1">
      ...
      <Card>
        <CardHeader>
          <div class="flex justify-between items-center">
            <CardTitle>Detalles de Atencion</CardTitle>
          </div>
        </CardHeader>
        <CardContent>
          <div v-if="incidencia.detalles && incidencia.detalles.length > 0" class="space-y-4">
            <div v-for="detalle in incidencia.detalles" :key="`${detalle.idincidencia}-${detalle.id}`" class="p-4 border rounded-lg hover:bg-gray-50">
              <div class="flex justify-between items-start mb-3">
                <div>
                  <p class="text-sm text-gray-600">Iniciado: {{ formatDate(detalle.fecha_inicio) }}</p>
                </div>
                <span :class="getEstadoBadgeClass(detalle.estado_atencion.id)" class="px-3 py-1 rounded-full text-xs font-semibold">
              ...
              <div class="grid grid-cols-3 gap-4 text-sm">
                <div>
                  <Label class="text-gray-600 font-semibold">Tecnico</Label>
                  <p class="text-gray-900">{{ detalle.empleadoInformatica?.nombre || 'Sin asignar' }}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p class="text-gray-900">{{ detalle.fecha_cierre ? formatDate(detalle.fecha_cierre) : 'Abierto' }}</p>
                </div>
              </div>
            </div>
          </div>
          <div v-else class="text-center py-8 text-gray-600">
            ...
            <p>No hay detalles de atencion registrados</p>
          </div>
        </CardContent>
      </Card>
    </div>
...
const eliminarIncidencia = async () => {
  if (!confirm('Estas seguro de que deseas eliminar esta incidencia?')) return

  try {
    const response = await fetch(`/incidencias/${incidencia.value.id}`, {
      method: 'DELETE',
      headers: {
